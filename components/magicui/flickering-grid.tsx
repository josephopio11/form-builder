  ...
  const memoizedColor = useMemo(() => {
    const toRGBA = (color: string) => {
      ...
      if (color.startsWith('hsl')) {
        ...
        const data = Array.from(ctx.getImageData(0, 0, 1, 1).data) // Convert to array
        const [r, g, b] = data // Destructure from the array
        return `rgba(${r}, ${g}, ${b},`
      }
      ...
      ctx.fillStyle = color
      ctx.fillRect(0, 0, 1, 1)
      const [r, g, b] = Array.from(ctx.getImageData(0, 0, 1, 1).data) // Convert to array
      return `rgba(${r}, ${g}, ${b},`
    }
    return toRGBA(color)
  }, [color])
